...
from override_components import override_component_attrs #Änderung AS
    ...
    if solve_opts.get('load_shedding'):
        if 'Load' in n.carriers.index:
            ...
            n.madd("Generator", n.buses.index, " load",
                   ...
                   marginal_cost=1e5, # Eur/kWh #Änderung AA, davor 1e3
                   # intersect between macroeconomic and surveybased
                   # willingness to pay
                   # http://journal.frontiersin.org/article/10.3389/fenrg.2015.00055/full
                   p_nom=1e4 # kW #Änderung AA, davor 1e9
                   )
...
def add_CCL_constraints(n, config, PYROLYSIS, **kwargs): #Änderung AS , PYROLYSIS, **kwargs #TODO PYROLYSIS wirklich notwendig
    ...
    if any(n.storage_units.p_nom_extendable == True):
        ...
        for j in list(agg_p_nom_minmax.index):
            if j[1] not in list(n.generators[n.generators.p_nom_extendable].carrier) and j[1] not in list(n.links[n.links.p_nom_extendable].carrier) : #Änderung AS added:  and i[1] not in list(n.links[n.links.p_nom_extendable].carrier) ; i durch j ersetzt
                logger.info("Droping {} from yearly constraints".format(j))
                agg_p_nom_minmax.drop(j, inplace=True)
    ...
    # Änderung AS:
    if PYROLYSIS==1: 
        agg_p_nom_minmax, p_nom_per_cc = pyrolysis.add_CCL_constraints_pyro(n,agg_p_nom_minmax, 
                                                                            p_nom_per_cc, 
                                                                            **kwargs) 
...
def extra_functionality(n, snapshots, **kwargs): #Änderung AS
    """
    Collects supplementary constraints which will be passed to ``pypsa.linopf.network_lopf``.
    If you want to enforce additional custom constraints, this is a good location to add them.
    The arguments ``opts`` and ``snakemake.config`` are expected to be attached to the network.
    """
    print(kwargs) #Änderung AS
    opts = n.opts
    config = n.config
    ...
    if 'CCL' in opts and n.generators.p_nom_extendable.any():
        add_CCL_constraints(n, config, **kwargs) #Änderung AS add , PYROLYSIS=kwargs.get("PYROLYSIS"),**kwargs
    add_battery_constraints(n)


def solve_network(n, config, solver_log=None, opts='', **kwargs): #Änderung AS
    ...
    if config['solving']['options'].get('skip_iterations', False):
        network_lopf(n, solver_name=solver_name, solver_options=solver_options,
                     extra_functionality=extra_functionality, **kwargs) #Änderung AS
    else:
...
if __name__ == "__main__":
    ...
    with memory_logger(filename=getattr(snakemake.log, 'memory', None),
        ...
        # overrides = override_component_attrs(snakemake.input.overrides) #new Änderung AS aktivieren wenn snakemake benutzt wird
        overrides = override_component_attrs(r'C:\Users\asandhaa\Desktop\pypsa-eur\MyPyPSA-biochar\pypsa-eur\data') # Änderungen AS
